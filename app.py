...
shutil.rmtree(job_desc_dir)
os.makedirs(job_desc_dir)

#Upload file
jobs = st.file_uploader("Upload Job Descriptions",
        type=["txt","docx","pdf","png","jpg","jpeg"],
        accept_multiple_files = True)   
if jobs is not None:
    for job in jobs:
        with open(os.path.join(job_desc_dir,job.name),"wb") as f:
...
if resumes is not None:
    for resume in resumes:
        with open(os.path.join(resume_dir,resume.name),"wb") as f:
...
fig1.update_layout(title="Top Ranked Resumes", width=700, height=400)
st.write(fig1)
